...
namespace TShock.Hooks.Player
{
	public class PlayerHooks
	{
		...
		private void ServerHooks_Connect(int who, HandledEventArgs args)
		{
		...
		private void NetHooks_GreetPlayer(int who, HandledEventArgs arg2)
		{
			var e = new PlayerEventArgs(new TShockPlayer(who));
			Greet.Invoke(this, e);
		...
		private void ServerHooks_Join(int arg1, HandledEventArgs arg2)
		{
		...
		private void ServerHooks_Leave(int who)
		{
			var e = new PlayerEventArgs(new TShockPlayer(who));
			Leave.Invoke(this, e);
		}

		private void ServerHooks_Chat(messageBuffer msg, int who, string text, HandledEventArgs args)
		{
			var player = new TShockPlayer(who);
			if (IsCommand(player, text))
				return;
			var e = new PlayerChatEventArgs(player, text);
			Chat.Invoke(this, e);
			args.Handled = e.Handled;
		}
		...
		public bool IsCommand(IPlayer player, string text)
		{
			if (text.StartsWith("/"))
			{
